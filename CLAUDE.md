...
- `ourList/$pushId/addedBy`, `tonightsDinner/$dateKey/proposedBy` and `counteredBy`: writer-claim fields. Pattern: `must equal auth.uid on creation-or-change, OR unchanged from prior value`. Blocks rewrites of authorship while still permitting sibling-field updates (e.g. flipping `done`/`status`) where the whole node re-validates. `counteredBy` additionally requires the uid be a current couple member.
...
- **Rule tightening — writer-claim fields** — `ourList.addedBy`, `tonightsDinner.proposedBy`, and `tonightsDinner.counteredBy` now enforce `auth.uid === newData.val()` on creation/change, or `newData.val() === data.val()` if unchanged. Previously they either accepted any string on update (addedBy) or any string at all (proposedBy, counteredBy) — a data-integrity hole. `counteredBy` additionally requires the uid be a current couple member. Sibling-field updates (toggling `done`, flipping status to `agreed`) still pass because unchanged values validate against the "unchanged" branch.
- **LDR hides Together-only notif toggles** — `initNotificationPrefs()` in `notifications.js` hides the entire `.settings-row-new` for `dateNight`, `dnHint`, `dnGuess`, `dnCorrect`, `dnReveal`, `tonightsMood`, `tonightsDinner`, and `listItemAdded` when `state.coupleType !== 'together'`. Purely visual — stored pref values are untouched so a Together → LDR → Together round-trip preserves prior settings. `applyMode()` calls `R.initNotificationPrefs()` on every mode flip to re-run the hide/show logic live.
